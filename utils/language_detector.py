...
LANGUAGE_PATTERNS = {
    ...
    "Kotlin": [
        ...
        (r"\bval\s+\w+\s*[:=]", 3),
        (r"\bvar\s+\w+\s*[:=]", 2),
        (r"\bcompanion\s+object\b", 5),
        (r"\bwhen\s*\(", 3),
        (r"!!\.", 4),
        (r"\?\.", 2),
        (r"\boverride\s+fun\b", 4),
        (r"^\s*package\s+[\w.]+$", 2),
        (r"\bimport\s+kotlin\.", 4),
        (r"\bimport\s+android\.", 3),
        (r"\bdata\s+class\s+\w+", 5),
    ],
    "Swift": [
        (r"\bfunc\s+\w+\s*\(", 4),
        (r"\b(var|let)\s+\w+\s*:\s*\w+", 3),
        (r"\bimport\s+(UIKit|Foundation|SwiftUI)\b", 5),
        (r"@(objc|IBOutlet|IBAction|State|Published)\b", 5),
        (r"\bguard\s+let\b", 4),
        (r"\bif\s+let\b", 3),
        (r"\bstruct\s+\w+\s*:\s*\w+", 3),
        (r"\bclass\s+\w+\s*:\s*(NSObject|UIViewController)\b", 5),
        (r"\bprint\(", 1),
    ],
    "Dart": [
        (r"\bimport\s+['\"]package:flutter/", 6),
        (r"\bclass\s+\w+\s+extends\s+(StatelessWidget|StatefulWidget)\b", 6),
        (r"\bWidget\s+build\s*\(\s*BuildContext\b", 5),
        (r"^\s*void\s+main\s*\(\s*\)\s*\{", 3),
        (r"@override\b", 2),
        (r"\bfinal\s+\w+\s*=", 2),
        (r"\bStatelessWidget\b|\bStatefulWidget\b", 4),
        (r"\bruntimeType\b", 3),
    ],
}
